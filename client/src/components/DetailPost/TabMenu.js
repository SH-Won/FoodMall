...
const TabMenu = ({ match }) => {
    ...
    return (
        <ul className="tab-menu" onClick={handleClick} ref={list}>
            <li className="tab-item" dataset="/">
                <Link to={`${match.url}`}>상세정보</Link>
            </li>
            <li className="tab-item" dataset="board">
                <Link to={`${match.url}/board`}>제품목록</Link>
            </li>
            <li className="tab-item" dataset="comment">
                <Link to={`${match.url}/comment`}>댓글</Link>
            </li>
        </ul>
    );
};
